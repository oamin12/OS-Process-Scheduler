...
void erase_node_memorystart(struct Queue *q, int start_index)
{
    struct Node *ptr = q->Head;

    if(ptr == NULL)
        return;
    else if(ptr->memory_index.first == start_index)
    {
        q->Head = ptr->next;
        free(ptr);
        return;
    ...
    while(ptr != NULL)
    {
        if(ptr->memory_index.first == start_index)
        {
...
void printqueue_memory(struct Queue *q)
{
    struct Node *p = q->Head;
    while (p != NULL)
    {
