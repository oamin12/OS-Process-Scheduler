...
void DeallocateProcessToMemory(int id)
{
    ...
    pair1.second = pcb_table[id].memoryStart + power(2, n+3) - 1;

    struct Node *temp = newNode_memory(pair1.first, pair1);
    fprintf(memlog,"At time %d freed %d bytes from process %d from %d to %d\n",getClk(),pcb_table[id].memorySize,id,pair1.first, pair1.second);
...
void sortBuddy(int q_num)
{
    struct Node* mytemp = peek_queue(free_memory[q_num]);
    
    while(mytemp != NULL)
    {

        int memorySize = mytemp->memory_index.second -  mytemp->memory_index.first + 1;
        int n = getBuddySize(memorySize);
        int buddyNumber, buddyAddress;

        
        buddyNumber = mytemp->memory_index.first / memorySize;

        if (buddyNumber % 2 != 0)
            buddyAddress = mytemp->memory_index.first - power(2, n+3);
        else
            buddyAddress = mytemp->memory_index.first + power(2, n+3);
        ...
        while(ptr != NULL)
        {
            if (ptr->memory_index.first == buddyAddress)
            {
                ...
                if (buddyNumber % 2 == 0)
                {
                    //-------------------------------------------
                    struct pair pair2;
                    pair2.first = mytemp->memory_index.first;
                    pair2.second = mytemp->memory_index.first + 2 * (power(2, n+3)) - 1; //ptr->memory_index.second;

                    struct Node *temp = newNode_memory(mytemp->memory_index.first, pair2);
                    //--------------------------------------------

                    enQueue(free_memory[n+1], temp);
                    printf("buddyAddress = %d  q_num = %d", buddyAddress, n);
                    printf("-------------------- 111 Merging of blocks starting at (%d, %d) to (%d, %d) \n", pair2.first, mytemp->memory_index.second, buddyAddress,pair2.second);
                ...
                else
                {
                    //-------------------------------------------
                    struct pair pair2;
                    pair2.first = buddyAddress; 
                    pair2.second = buddyAddress + 2 * (power(2, q_num+3)) - 1;// pcb_table[id].memoryEnd;

                    struct Node *temp = newNode_memory(mytemp->memory_index.first, pair2);
                    //--------------------------------------------

                    enQueue(free_memory[q_num+1], temp);
                    printf("-------------------- 222 Merging of blocks starting at %d to %d \n", pair2.first, pair2.second);
                }
                
                erase_node_memorystart(free_memory[n], mytemp->memory_index.first);
                erase_node_memorystart(free_memory[n], buddyAddress);
